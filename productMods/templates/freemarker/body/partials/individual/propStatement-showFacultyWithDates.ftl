...
<#local linkedIndividual>
    <#if (statement.res?? && !(statement.date??)) >
        <a href="${profileUrl(statement.uri("faculty"))}" title="class">${statement.name!}</a>, ${statement.label}
    <#elseif (statement.res?? && statement.date??)>
    	<a href="${profileUrl(statement.uri("faculty"))}" title="class">${statement.name!}</a>, ${statement.label} (${statement.date})
    <#else>
